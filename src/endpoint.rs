...
use crate::{connection::Connection, iroh_error::IrohError};
...
impl Endpoint {
    ...
    pub async fn connect(self: Arc<Self>, node_addr: &str, alpn: &[u8]) -> Result<Connection, IrohError> {
    ...
    pub async fn accept(self: Arc<Self>) -> Result<Connection, IrohError> {
        let incoming = self.0.accept().await.ok_or_else(|| anyhow!("This endpoint has been closed"))?;
