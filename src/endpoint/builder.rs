...
impl EndpointBuilder {
    ...
    pub fn proxy_from_env(self: Arc<Self>) -> Result<Arc<Self>, IrohError> {
        self.apply(|builder| builder.proxy_from_env());
        Ok(self)
    }

    pub fn keylog(self: Arc<Self>, keylog: bool) -> Result<Arc<Self>, IrohError> {
        self.apply(|builder| builder.keylog(keylog));
        Ok(self)
    }
...
impl EndpointBuilder {
    fn apply(&self, f: impl FnOnce(iroh::endpoint::Builder) -> iroh::endpoint::Builder) {
        self.0.lock().unwrap().apply(f);
    }
}
